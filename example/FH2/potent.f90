...
!> Since DVR grid don't have the boundary point
        ...
        do i = 1, nGird
            if (grid(i) >= rangeNoAbs) then
                if (trim(Vabs%type) == 'EXP') then 
                    VabsMat(i) = Vabs_EXP(Cabs, rangeNoAbs, rangeAll, grid(i))
                else if (trim(Vabs%type) == 'TF') then
                    VabsMat(i) = Vabs_TF(massTot, rangeNoAbs, rangeAll, grid(i))
                else 
                    write(outFileUnit,'(1x,a)') 'Error: unknown Vabs type in setVabs!'
                    write(outFileUnit,'(1x,a)') 'POSITION: potent.f90, subroutine setVabs()'
                    stop
                end if
...
!> Vabs in asymptotic
        ...
        write(outFileUnit,'(1x,a)') ''
!        do i = 1, IALR%nZ_IA
!            write(120,*) Z_IALR(i), asy_ZFabs(i)
!        end do
        ...
        do ith = 1, nth
            do ir = 1, nr
                do iZ = 1, nZ
                    Z = ZGrid(iZ)
                    r = rGrid(ir)
                    !> Important, the grids of theta is cos(theta)
                    theta = acos(thGrid(ith))
...
!> Z grid in long-range region
        ...
        allocate(lr_Vdia(lr_nZ,IALR%vlr,IALR%lr_nA))

        write(outFileUnit,'(1x,a)') '=====> Potential information <====='
        write(outFileUnit,'(1x,a)') ''
...
!> interaction region
        ...
        !    do ir = 1, IALR%vint
        !        do iZ = 1, IALR%nZ_I
        !            write(123,*) Z_IALR(iZ), r_Int(ir), int_Vdia(1,1,iZ,ir,ith)*au2ev
        !        end do
        !    end do
        !end do
!> asymptotic region
            ...
            do  iZ = 1, lr_nZ
                ...
                do ir = 1, IALR%vlr
                    ...
                    do ith = 1, IALR%lr_nA
                        ...
                        lr_Vdia(iZ, ir, ith) = min((Vdia(1,1)-VBC), VMaxCut)
                    end do
                end do 
            end do
...
!> ------------------------------------------------------------------------------------------------------------------ <!
    real(f8) function Vabs_EXP(Cabs, x0, xmax, x) result(V)
        implicit none
        real(f8), intent(in) :: Cabs, x0, xmax, x

        V = dexp(-Cabs * ((x-x0)/(xmax-x0))**2)

    end function Vabs_EXP

    real(f8) function Vabs_TF(tms, x0, xmax, x) result(V)
        implicit none
        real(f8), intent(in) :: tms, x0, xmax, x
        real(f8) :: c, y, z 

        c = 2.62206_f8
        z = c*(x-x0)/(xmax-x0)
        y = 4.0_f8/(c-z)**2 + 4.0_f8/(c+z)**2 - 8.0_f8/(c**2)
        V = dexp(-y/(2.0_f8*tms) * ((2.0_f8*pi)/(xmax-x0))**2)

    end function Vabs_TF
!> ------------------------------------------------------------------------------------------------------------------ <!
