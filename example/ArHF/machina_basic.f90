...
!    use MKL_DFTI
    ...
    interface BinReadWrite
        ...
        module procedure realBinReadWrite3D
...
!> ------------------------------------------------------------------------------------------------------------------ <!
    subroutine realBinReadWrite1D(file, data, action)
        ...
        if (action == 'read') then
            open(newunit=streamUnit, file=trim(file), access='stream', form='unformatted', status='old')
            read(streamUnit) data
            close(streamUnit)
        else if (action == 'write') then
            open(newunit=streamUnit, file=trim(file), access='stream', form='unformatted', status='replace')
...
!> ------------------------------------------------------------------------------------------------------------------ <!
    subroutine realBinReadWrite3D(file, data, action)
        implicit none
    ...
    end subroutine realBinReadWrite3D
!> ------------------------------------------------------------------------------------------------------------------ <!
