...
!> Product diatomic vib-rot energy and wavefunction in product coordinates
    real(f8), allocatable, public :: Evj_AB(:,:,:), Evj_AC(:,:,:)
...
!> Find vmax for channel 1
        ...
        if (channel1%vmax < vmax) then
            write(outFileUnit,'(1x,a,i4,a,i4)') 'Warning: channel 1 vmax increased from ', &
...
!> Find jmax for channel 1
        ...
        if (channel1%jmax < jmax) then
            write(outFileUnit,'(1x,a,i4,a,i4)') 'Warning: channel 1 jmax increased from ', &
...
!> Find vmax for channel 2
            ...
            if (channel2%vmax < vmax) then
                write(outFileUnit,'(1x,a,i4,a,i4)') 'Warning: channel 2 vmax increased from ', &
...
!> Find jmax for channel 2
            ...
            if (channel2%jmax < jmax) then
                write(outFileUnit,'(1x,a,i4,a,i4)') 'Warning: channel 2 jmax increased from ', &
        ...
        if (type == 'A+BC->C+AB') then 
            ...
            do iPES = 1, nPES
                do j = channel1%jmin, channel1%jmax, channel1%jinc
                    ...
                    !call phase(channel1%nr, DVRWF)
                    do ir = 1, channel1%nr
                        call phaseTrans(channel1%nr,DVRWF(:,ir))
                    end do
            ...
            do iPES = 1, nPES
                do j = channel2%jmin, channel2%jmax, channel2%jinc
                    ...
                    do ir = 1, channel2%nr
                        call phaseTrans(channel2%nr,DVRWF(:,ir))
                    end do
...
!> ------------------------------------------------------------------------------------------------------------------ <!
    subroutine interCoorWFvj(type, ichoice)
        ...
        if (type == 'A+BC->C+AB') then 
            ...
            allocate(intAB_TIDWF(nEtot,channel1%nIC,initWP%Kmin:Kmax,nPES))
            intAB_TIDWF = imgZero
            ICWFvj_AB = 0.0_f8
            do iPES = 1, nPES 
            ...
            open(newunit=normUnit, file=fileName, status='replace', action='write')
            ...
            allocate(intAC_TIDWF(nEtot,channel2%nIC,initWP%Kmin:Kmax,nPES))
            intAC_TIDWF = imgZero
            ICWFvj_AC = 0.0_f8
            do iPES = 1, nPES 
...
!> ------------------------------------------------------------------------------------------------------------------ <!
    subroutine setTransKMat(type)
        ...
        if (type == 'A+BC->C+AB') then 
            ...
            allocate(TKMat_AB(initWP%Kmin:Krmax,channel1%Kmin:Kpmax,channel1%nIC))
            do iIC = 1, channel1%nIC
                do Kp = channel1%Kmin, Kpmax
                    do Kr = initWP%Kmin, Krmax
                        beta = interCoor_AB(iIC,6)
                        TKMat_AB(Kr,Kp,iIC) = dm(initWP%Jtot, Kp, Kr, beta) &
                                            + dm(initWP%Jtot, Kp, -Kr, beta) &
                                            * initWP%tpar*(-1.0_f8)**(Kr)
                        if (Kr == 0) TKMat_AB(Kr,Kp,iIC) = TKMat_AB(Kr,Kp,iIC)/dsqrt(2.0_f8)
                        if (Kp == 0) TKMat_AB(Kr,Kp,iIC) = TKMat_AB(Kr,Kp,iIC)/dsqrt(2.0_f8)
                    end do
                end do
            end do
            ...
            allocate(TKMat_AC(initWP%Kmin:Krmax,channel2%Kmin:Kpmax,channel2%nIC))
            do iIC = 1, channel2%nIC
                do Kp = channel2%Kmin, Kpmax
                    do Kr = initWP%Kmin, Krmax
                        beta = interCoor_AC(iIC,6)
                        TKMat_AC(Kr,Kp,iIC) = dm(initWP%Jtot, Kp, Kr, beta) &
                                            + dm(initWP%Jtot, Kp, -Kr, beta) &
                                            * initWP%tpar*(-1.0_f8)**(Kr)
                        if (Kr == 0) TKMat_AC(Kr,Kp,iIC) = TKMat_AC(Kr,Kp,iIC)/dsqrt(2.0_f8)
                        if (Kp == 0) TKMat_AC(Kr,Kp,iIC) = TKMat_AC(Kr,Kp,iIC)/dsqrt(2.0_f8)
                    end do
                end do
            end do
        ...
        if (ichoice == 1) then
            do ith = 1, IALR%int_nA
                do iZ = 1, IALR%nZ_I
                    ...
                    if (rij > r_Int(1) .and. rij < r_Int(Vabs%nrint)) then
...
!> ------------------------------------------------------------------------------------------------------------------ <!
    subroutine setUij(ichoice, nIC, interCoor, idXY, Uij)
        ...
        real(f8), allocatable :: rDVR2FBR(:,:)
        real(f8), allocatable :: FBR_WF(:,:), DVR_WF(:)
        real(f8) :: rFact, Zfact, rij, gij
...
!> Set the Uij transformation matrix
        ...
        if (ichoice == 1) then
            allocate(DVR_WF(IALR%nr_DVR))
            allocate(FBR_WF(IALR%nr_DVR,IALR%vint))
            allocate(rDVR2FBR(IALR%nr_DVR,IALR%nr_DVR))
            call setDVR2FBR(IALR%nr_DVR, rDVR2FBR)

            call dgemm('N','N',IALR%nr_DVR,IALR%vint,IALR%nr_DVR,1.0_f8, &
                        rDVR2FBR,IALR%nr_DVR,int_PO2DVR,IALR%nr_DVR,0.0_f8,FBR_WF,IALR%nr_DVR)   
            
            Uij = 0.0_f8
            do iIC = 1, nIC 
                rij = interCoor(iIC,2)
                do iU = 1, IALR%nr_DVR
                    DVR_WF(iU) = dsqrt(2.0_f8/(IALR%r_range(2)-IALR%r_range(1))) * &
                                dsin(iU*pi*(rij-IALR%r_range(1))/(IALR%r_range(2)-IALR%r_range(1)))
                end do
                call dgemv('T',IALR%nr_DVR,IALR%vint,1.0_f8,FBR_WF,IALR%nr_DVR,DVR_WF,1,0.0_f8,Uij(1,iIC),1)
                gij = interCoor(iIC,7)
                do iU = 1, IALR%vint
                    Uij(iU,iIC) = Uij(iU,iIC)*gij
                end do
            end do
            deallocate(DVR_WF)
            deallocate(FBR_WF)
